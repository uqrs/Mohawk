...
################################################################################
# Read the comments before editing. Please. they are these for a reason.
################################################################################
...
# down to escaping single quotes: ' => '\''
...
# gsub(), and sub() into its literal form.
...
# `gawk [varlist] [eot] [respondto] [addon] -f 'file' -e '{ print"";fflush() }' [end]`
...
#
# There are two more expressions (-e options) plugin_header() supplies:
# The first, [eot], is a single statement that, when it detects a single EOT
# (End Of Transmission, ASCII 004) has been sent, the plugin will exit,
# jumping to the END block so that it may do some cleaning up (e.g. closing
# files, serialising data). Normally, only power_off() sends singular EOTs
# to plugin files. This expression would look like:
# `-e '$0 ~ /^\004$/ { exit; }'`
#
...
function power_off (channel)
{
   ...
   for ( plugin in _proclist[channel] ) {
      ...
      if (_byproc[_proclist[channel][plugin]] ~ /^\s*@?\s*$/) {
         run_proc("\004",_proclist[channel][plugin]);
